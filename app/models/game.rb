...
#### Added by Matt Arrick -- 8/30/19
  ...
  def populate_game             # Create white pieces and place them in thier inital space
    ...
    Rook.create(piece_type: 'Rook', x_pos: 0, y_pos: 0, color: true, game_id: self.id, player_id: self.white_player_id)
    Rook.create(piece_type: 'Rook', x_pos: 7, y_pos: 0, color: true, game_id: self.id, player_id: self.white_player_id)

    # Create White Knights -- coordinates are on the first row (0,1 and 0,6)
    Knight.create(piece_type: 'Knight', x_pos: 1, y_pos: 0, color: true, game_id: self.id, player_id: self.white_player_id)
    Knight.create(piece_type: 'Knight', x_pos: 6, y_pos: 0, color: true, game_id: self.id, player_id: self.white_player_id)

    # Create White Bishops -- coordinates are on the first row (0,2 and 0,5)
    Bishop.create(piece_type: 'Bishop', x_pos: 2, y_pos: 0, color: true, game_id: self.id, player_id: self.white_player_id)
    Bishop.create(piece_type: 'Bishop', x_pos: 5, y_pos: 0, color: true, game_id: self.id, player_id: self.white_player_id)

    # Create White Queen and King -- Queen coordinates are 0,3.  King coordinates are 0,4
    Queen.create(piece_type: 'Queen', x_pos: 3, y_pos: 0, color: true, game_id: self.id, player_id: self.white_player_id)
    King.create(piece_type: 'King', x_pos: 4, y_pos: 0, color: true, game_id: self.id, player_id: self.white_player_id)
    ...
    Rook.create(piece_type: 'Rook', x_pos: 0, y_pos: 7, color: false, game_id: self.id, player_id: self.black_player_id)
    Rook.create(piece_type: 'Rook', x_pos: 7, y_pos: 7, color: false, game_id: self.id, player_id: self.black_player_id)

    # Create Black Knights -- coordinates are on the last row (7,1 and 7,6)
    Knight.create(piece_type: 'Knight', x_pos: 1, y_pos: 7, color: false, game_id: self.id, player_id: self.black_player_id)
    Knight.create(piece_type: 'Knight', x_pos: 6, y_pos: 7, color: false, game_id: self.id, player_id: self.black_player_id)

    # Create Black Bishops -- coordinates are on the last row (7,2 and 7,5)
    Bishop.create(piece_type: 'Bishop', x_pos: 2, y_pos: 7, color: false, game_id: self.id, player_id: self.black_player_id)
    Bishop.create(piece_type: 'Bishop', x_pos: 5, y_pos: 7, color: false, game_id: self.id, player_id: self.black_player_id)

    # Create Black Queen and King -- Queen coordinates are 7,3.  King coordinates are 7,4
    Queen.create(piece_type: 'Queen', x_pos: 3, y_pos: 7, color: false, game_id: self.id, player_id: self.black_player_id)
    King.create(piece_type: 'King', x_pos: 4, y_pos: 7, color: false, game_id: self.id, player_id: self.black_player_id)
